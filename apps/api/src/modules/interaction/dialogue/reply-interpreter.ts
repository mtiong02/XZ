...
const CONFIRM_PATTERN =
  /^(?:是的?|对(?:对|的)?|确认(?:是)?|没错|好的?|好嘞|可以(?:的)?|行(?:行|的)?|(?:没|没有)问题(?:的)?|加了|添加了|用掉了|用了|吃了|吃完|就这些|就这个|没有意见|就酱|就这样|可以执行|ok|yes|correct|right|yep|sure|我说(?:的)?(?:是)?)(?:[\s,，.！!]*)(?:是|是的?|对|对的?|确认|好的?|行|行的|没问题|用掉了?|用了?|用|吃了?|吃完(?:了)?|删了|删除了?|加了|添加了?|放了|移了|记录了|我吃完(?:了)?|帮我(?:添|添加|加|加上|用|用掉|记录|确认|执行)(?:一下)?|请帮我(?:添|添加|加|加上|用|用掉|记录|确认|执行)(?:一下)?|了|吧|呢|呀|啊|准|正确)*$/i;
const REJECT_PATTERN =
  /^(?:不对|不是(?:的)?|不要|不想(?:要)?|取消|算了|错了|不用(?:了)?|重来|结束|退出|不(?:加|用|买|做)(?:了)?|打断|停(?:止)?|都不是|不聊了|散了吧|差不多了|我先走了|我先忙了|没有|不是这个意思|听错了|搞错了|你说错了|你听错了|理解错了|不对不对|不是不是|别记了|不记了|不要记|no|cancel|wrong|nope)+(?:了|吧|啊|的|呀)?$/i;
const SKIP_PATTERN =
